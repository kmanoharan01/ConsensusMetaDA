...
To illustrate functionality of ```ConsensusMetaDA```, we will utilise microbiome biom format data from the ```GWMC_HOT_COLD``` and annotation libraries as follows. Begin by installing and attaching data from these libraries as follows:
...
otu_mat <- as(otu_table(ecc_plaque), "matrix")
sample_df <- as(sample_data(ecc_plaque), "data.frame")
tax_mat <- as(tax_table(ecc_plaque), "matrix")

#otu_final <- data.frame(sampleID = rownames(otu_mat), otu_mat)

#write.table(otu_mat, file = "ecc_plaque_Otu.txt",  col.names = TRUE, sep = "\t", quote = FALSE)

#write.table(sample_df, file = "ecc_plaque_sample_table.txt", col.names = TRUE, sep = "\t", quote = FALSE)

write.table(tax_mat, file = "ecc_plaque_tax_table.txt", col.names = TRUE, sep = "\t", quote = FALSE)

################

#system("module load python3/3.10.4")

#system("~/.local/bin/biom convert -i ecc_plaque_Otu.txt   -o ecc_plaque_Otu.biom --table-type='OTU table' --to-json")

ecc_plaque_biom <- "ecc_plaque_Otu.biom"

ecc_plaque_sample_table <- "ecc_plaque_sample_table.txt"

ecc_plaque_tax <- tax_mat
...
* ```ID``` - Identifier
* ```AveExpr``` - Average Expression (average of edgeR, DESeq2, ALDEx2, MetagenomeSeq and ADAPT)
...
* ```ALDEx2_rank``` - rank of the p-value obtained by ALDEx2
* ```MetagenomeSeq_rank``` - rank of the p-value obtained by MetagenomeSeq
* ```ADAPT_rank``` - rank of the p-value obtained by ADAPT
...
ecc_saliva_plots <- OTU_plots(ecc_saliva_build_OTU_counts)
