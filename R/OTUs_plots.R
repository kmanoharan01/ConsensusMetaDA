...
#' and generates publication-ready plots comparing groups.
#'
#' \itemize{
#'   \item Alpha diversity plots and tables for all taxonomic levels
#'   \item PCoA/MDS ordination plot
#'   \item Stacked bar plots showing taxonomic composition by group
#'   \item Bidirectional plots comparing abundance between groups
...
#' @param group_var Character. Name of the sample-metadata column that holds the
#'   grouping variable to compare (e.g. "Group"). Default = "Group".
...
#' \itemize{
#'   \item Sample metadata must contain the column named by \code{group_var}
...
OTUs_plots <- function( build_OTU_counts_output = NULL,
  ...
  ## validate the grouping column exists in the sample metadata
  .meta_cols <- colnames(as(phyloseq::sample_data(build_OTU_counts_output), "data.frame"))
  if (!group_var %in% .meta_cols) {
    stop(sprintf("group_var '%s' not found in sample metadata. Available columns: %s",
                 group_var, paste(.meta_cols, collapse = ", ")))
  }
  ...
  bidirectional_plot <- function(physeq2, tax_level, treatment, other_treatment) {
    ...
    # Filter for only the two treatments of interest (uses the chosen group column)
    tax_data <- tax_data %>% filter(.data[[group_var]] %in% c(treatment, other_treatment))
    ...
    tax_summary <- tax_data %>%
      group_by(.data[[group_var]], !!sym(tax_level)) %>%
      summarize(
    ...
    adults_negative_summary <- tax_summary %>% filter(.data[[group_var]] == treatment)
    adults_positive_summary <- tax_summary %>% filter(.data[[group_var]] == other_treatment)
  ...
  # rarefaction expects samples as rows; transpose if taxa are rows
  if (phyloseq::taxa_are_rows(build_OTU_counts_output)) {
    Data_t <- t(otu_mat)
  } else {
    Data_t <- otu_mat
  }
  ...
  pcoa_plot <- plot_ordination(build_OTU_counts_output,
                               ordinate(build_OTU_counts_output, "PCoA"),
                               color = group_var, label = "SampleID") + theme_bw()
  ...
  for (tax_level in taxonomic_levels) {
    ...
    write.table(shannon_diversity,
                file = paste0(group_var, "_alpha_Diversity_", tax_level, ".txt"),
  ...
  for (tax_level in taxonomic_levels) {
    ...
    richness_plot <- plot_richness(tax_glom_physeq, measures = c("Shannon"),
                                   x = group_var, color = group_var) +
    ...
    # Store the plot (kept under $AlphaDiv$tax_level for backward compatibility)
    OTU_plot_list$AlphaDiv$tax_level <-  richness_plot
    
    ggsave(paste0(group_var, "_alpha_Diversity_plot_shannon_", tax_level, ".pdf"),
           plot = richness_plot, width = 8, height = 6)
  }
  ...
  ## facet by the chosen group column (built with as.formula so group_var is honoured)
  facet_f <- stats::as.formula(paste("~", group_var))
  
  scale_plot_kingdom <- ggplot(data=build_OTU_counts_output_kingdom_data,aes(x=Sample,y=Abundance,fill=Kingdom)) + facet_grid(facet_f,scales="free") + geom_bar(aes(), stat="identity", position="stack") + scale_fill_manual(values = colors_to_use) + theme(  axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
  scale_plot_phylum <- ggplot(data=build_OTU_counts_output_phylum_data,aes(x=Sample,y=Abundance,fill=Phylum)) + facet_grid(facet_f,scales="free") + geom_bar(aes(), stat="identity", position="stack") + scale_fill_manual(values = colors_to_use) + theme(  axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
  scale_plot_class <- ggplot(data=build_OTU_counts_output_class_data,aes(x=Sample,y=Abundance,fill=Class)) + facet_grid(facet_f,scales="free") + geom_bar(aes(), stat="identity", position="stack") + scale_fill_manual(values = colors_to_use) + theme(  axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
  scale_plot_order <- ggplot(data=build_OTU_counts_output_order_data,aes(x=Sample,y=Abundance,fill=Order)) + facet_grid(facet_f,scales="free") + geom_bar(aes(), stat="identity", position="stack") + scale_fill_manual(values = colors_to_use) + theme(  axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
  scale_plot_family <- ggplot(data=build_OTU_counts_output_family_data,aes(x=Sample,y=Abundance,fill=Family)) + facet_grid(facet_f,scales="free") + geom_bar(aes(), stat="identity", position="stack") + scale_fill_manual(values = colors_to_use) + theme(  axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
  scale_plot_genus <- ggplot(data=build_OTU_counts_output_genus_data,aes(x=Sample,y=Abundance,fill=Genus)) + facet_grid(facet_f,scales="free") + geom_bar(aes(), stat="identity", position="stack") + scale_fill_manual(values = colors_to_use) + theme(  axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
  
  ggsave("scale_plot_kingdom.pdf", plot = scale_plot_kingdom, width = 15, height = 10)
  ggsave("scale_plot_phylum.pdf", plot = scale_plot_phylum, width = 15, height = 10)
  ...
  treat_list <- unique(as.character(sample_data(build_OTU_counts_output)[[group_var]]))
  
  all_comparisons_results <- list()
  contrast_list <- list()
  
  # Run every ordered pair and store bidirectional plots per taxonomic level
  for (treatment in treat_list) {
    for (other_treatment in treat_list) {
      if (treatment != other_treatment) {
        
        contrast_list <- c(contrast_list, list(c(group_var, treatment, other_treatment)))
        ...
        bd_levels <- c("Phylum", "Class", "Order", "Family", "Genus")
        
        for (tax_level in bd_levels) {
          try({
